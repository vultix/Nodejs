...
export class LeoCron {
	...
	private SYSTEM_TABLE = this.configure.resources.LeoSystem;
	...
	trigger(cron, callback) {
		...
		this.dynamodb.docClient.update(params, function(err) {
			callback(err, {
				refId: cron.id
			});
		});
	}
	schedule(id, duration): Promise<void> {
		if (duration == undefined) {
			duration = id;
			id = this.configure.registry.__cron && this.configure.registry.__cron.id
		}
		if (!id) {
		...
		if (typeof duration == "number" || duration instanceof Date || moment.isMoment(duration)) {
			...
			time = moment().add(moment.duration(duration)).valueOf();
		}
		var params = {
			TableName: this.CRON_TABLE,
			Key: {
	...
	checkLock(cron, runid, remainingTime, callback) {
		if (cron.time && (Date.now() - cron.time) > 2000) {
			callback("Old Run: Skipping");
		} else if (cron.ignoreLock) {
			callback();
		} else {
			var command: AWS.DynamoDB.DocumentClient.UpdateItemInput = {
				TableName: this.CRON_TABLE,
				Key: {
					id: cron.id
				},
			...
			this.dynamodb.docClient.update(command, (err, data) => {
				if (err && (err.code == "" || cron.force)) {
					...
					this.dynamodb.put(this.CRON_TABLE, cron.id, entry, function(err, data) {
						if (err) {
	...
	reportComplete(cron, runid, status, log, opts, callback) {
		...
		if (opts.forceComplete) {
			command = {
				...
				ExpressionAttributeValues: {
					":value": {
						...
						message: cron.message || this.configure.registry.cron_message || null
					}
				},
				"ReturnConsumedCapacity": 'TOTAL'
			};
			...
			this.configure.registry.cron_message = cron.message || this.configure.registry.cron_message || null;
			command.UpdateExpression += ", #msg = :msg";
			command.ExpressionAttributeNames["#msg"] = "message";
			command.ExpressionAttributeValues[":msg"] = this.configure.registry.cron_message;
		}
		...
		this.dynamodb.docClient.update(command, function(err, data) {
			// TODO: TS - This appears to be a bug
			// @ts-ignore
	...
	createLock(id, runid, maxDuration, callback) {
		id = "lock_" + id;
		this.dynamodb.docClient.put({
			TableName: this.SETTINGS_TABLE,
			// TODO: TS - I'm guessing this isn't necessary?
			// Key: {
			//     id: id
			// },
			Item: {
	...
	removeLock(id, runid, callback) {
		id = "lock_" + id;
		this.dynamodb.docClient.delete({
			TableName: this.SETTINGS_TABLE,
			Key: {
				id: id
			},
	...
	checkpoint(id, event, params?: any, callback?: any) {
		...
		var cronCheckpointCommand: AWS.DynamoDB.DocumentClient.UpdateItemInput = {
			TableName: this.CRON_TABLE,
			Key: {
				id: id
			},
		...
		if (type == "read" && !params.force && (!this.configure.registry ||
			!this.configure.registry.__cron || !this.configure.registry.__cron.force) && (params.expected || (this.configure.registry && this.configure.registry.__cron))) {
			var expected = params.expected || (this.configure.registry &&
				this.configure.registry.__cron &&
				this.configure.registry.__cron.checkpoints &&
				this.configure.registry.__cron.checkpoints[type] &&
				this.configure.registry.__cron.checkpoints[type][event] &&
				this.configure.registry.__cron.checkpoints[type][event].checkpoint) || undefined;
		...
		var updateInMemoryCheckpoint = () => {
			...
			var c = ["registry", "__cron", "checkpoints", type].reduce((o, f) => o[f] = o[f] || {}, this.configure);
			c[event] = checkpointData;
		};

		let checkpointLocation = (this.configure.registry && this.configure.registry.__cron && this.configure.registry.__cron.cploc);
		if (checkpointLocation) {
			cronCheckpointCommand.ExpressionAttributeNames[`#checkpoints`] = checkpointLocation;
			cronCheckpointCommand.ExpressionAttributeNames["#type"] = this.configure.registry.__cron.iid.toString();
		...
		this.dynamodb.docClient.update(cronCheckpointCommand, (err, data) => {
			if (err && err.code == "ConditionalCheckFailedException") {
				...
				this.dynamodb.get(this.CRON_TABLE, id, {}, (getErr, getData) => {
					// Err getting entry
					if (getErr) {
						...
						if (checkpointLocation) {
							entry[checkpointLocation] = {
								[this.configure.registry.__cron.iid]: {
						...
						this.dynamodb.put(this.CRON_TABLE, id, entry, function(err, data) {
							if (err) {
	...
	async getCheckpoint(botId, queue) {
		this.configure.registry = this.configure.registry || {};
		if (queue == undefined) {
			queue = botId;
			botId = (this.configure.registry.__cron && this.configure.registry.__cron.id);
		}
		let id = refUtil.refId(queue);
		if (!this.configure.registry.__cron || !this.configure.registry.__cron.checkpoints) {
			let data: any = await new Promise((resolve, reject) => {
				this.get(botId || this.configure.registry.__cron.id, {}, (err, data) => {
					if (err) {
						reject(err);
					} else {
						resolve(data);
			...
			this.configure.registry.__cron = this.configure.registry.__cron || data.__cron;
			this.configure.registry.__cron.checkpoints = data.__cron.checkpoints
		}
	...
	update(bot, callback) {
		return new Promise((resolve, reject) => {
			...
			this.dynamodb.merge(this.CRON_TABLE, bot.id, bot, done);
		});
	}
	subscribe(id, queue, data) {
		if (!id || !queue) {
	...
	getLastResult() {
		var r = this.configure.registry.__cron && this.configure.registry.__cron.lastResult;
		return (r != undefined && r != null) ? JSON.parse(zlib.gunzipSync(Buffer.from(r, "base64")).toString('utf8')) : null;
	}
	setMessage(message) {
		...
		this.configure.registry.cron_message = message;
	}
	getAttachedSystemByAlias(alias) {
	...
	get(id, opts, callback: DataCallback) {
		...
		this.dynamodb.get(this.CRON_TABLE, id, (err, data) => {
			if (err) {
				callback(err);
			} else {
				...
				if (opts.register) {
					process.__config = process.__config || this.configure;
					process.__config.registry = process.__config.registry || {};
					this.configure.registry = extend(true, process.__config.registry, this.configure.registry || {});
					var checkpointData = ["registry", "__cron", "checkpoints", "read"].reduce((o, f) => o[f] = o[f] || {}, this.configure);
			...
			if (Object.keys(systemIds).length) {
				this.dynamodb.docClient.batchGet({
					...
					if (err) {
						...
						var lookup = {}
						let response = data.Responses[this.SYSTEM_TABLE];
	...
	shouldRun(oldImage, newImage, cache, callback) {
	...
	hasMoreToDo(oldImage, newImage, key?: any) {
	...
	start(event, opts?: any, callback?: any) {
		this.configure.registry.__cron = event.__cron = event.__cron || {
		...
		let context = this.configure.registry.context;
		...
		let get__cron = (d) => {
			...
			this.get(event.id || event.botId, {}, (err, data) => {
				...
				let cron = this.configure.registry.__cron = event.__cron = data.__cron;
				cron.ts = Date.now();
				let index = cron.iid;
				this.configure.registry.__cron.__requestId = opts && opts.requestId || (Math.round(Math.random() * 10000000000000000)).toString();
				...
				if (!cron.checkpoints) {
					...
					this.dynamodb.docClient.update(command, function(err) {
						d(err);
					});
				}
			});
		...
		callback = callback || (() => {});
		if (this.configure.registry.__cron) {
			this.configure.registry.__cron.__requestId = opts && opts.requestId
		}
		...
		let doCheckLock = (resolve, reject, callback) => {
			if (event.__cron && opts.lock) {
				...
				this.checkLock(event.__cron, this.configure.registry.__cron.__requestId, opts.timeout || (context && context.getRemainingTimeInMillis && context.getRemainingTimeInMillis()) || 24 * 60 * 1000, function(lockErr, lockData) {
	...
	end(status, opts, callback): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			...
			let requestId = this.configure.registry.__cron && this.configure.registry.__cron.__requestId;
			this.reportComplete(this.configure.registry.__cron, requestId, (status != "complete") ? "error" : "complete", (status != "complete") ? status : '', {
				forceComplete: this.configure.registry.__cron.forceComplete
	...
	run(event, handler, callback) {
		...
		return this.start(event)
			...
			// TODO: TS - this is a bug, what is it?
			// @ts-ignore
			.then(() => leo.bot.end(e))
			.then(() => callback());
	}
	createBot(id, bot, opts?: any) {
		return new Promise((resolve, reject) => {
			bot = Object.assign({
				lambdaName: id,
				name: bot.name,
			...
			this.dynamodb.update(this.CRON_TABLE, {
				id: id
